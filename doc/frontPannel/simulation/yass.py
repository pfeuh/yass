...
def objectsAppend(object, comment):
    if comment != "":
        object.setComment(comment)
        objects.append(object)

def drawRectangle(c, x, y, w, h, comment=""):
    object = classObject.RECTANGLE(x, y, w, h)
    id = object.draw(c, DRAW_SCALE, outline=DRAW_COLOR)
    objectsAppend(object, comment)
    return id

def drawCircle(c, x, y, d, color=None, comment=""):
...
def drawText(c, x, y, text, font=None, fill=None, comment=""):
    if fill == None:
        fill = DRAW_COLOR
...
def drawLed(c, x, y, color=DEFAULT_LED_COLOR ):
    drawCircle(c, x, y, LED_DIAMETER, color, "led")
    
def drawKey(c, x, y, text="???", font=None):
    drawRectangle(c, x, y, KEY_WIDTH, KEY_HEIGHT, "key")
    kwds = {}
    if font != None:
        kwds["font"] = font
...
def drawDisplay(canvas):
    ...
    drawRectangle(canvas, x, y, DISPLAY_WIDTH, DISPLAY_HEIGHT, "display")
...
def drawEncoder(canvas):
    ...
    drawCircle(canvas, x, y, ENCODER_DIAMETER * 0.3, None, "encoder")
...
if __name__ == "__main__":
    ...
    for item in objects:
        ...
        if comment in ("led", "encoder"):
            x, y, d = item.getSpecs()
            print("%-8s(%.02f, %.02f) %.02f"%(comment, x, y, d) )
        elif comment in ("key", "display"):
            x, y, w, h = item.getSpecs()
            print("%-8s(%.02f, %.02f) %.02f x %.02f"%(comment, x, y, w, h) )
